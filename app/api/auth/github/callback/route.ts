...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const storedState =
    cookieStore.get(authMode ? "github_auth_state" : "github_oauth_state")
      ?.value ?? null;
  const storedRedirectTo =
    cookieStore.get(
      authMode ? "github_auth_redirect_to" : "github_oauth_redirect_to"
    )?.value ?? null;
  ...
  try {
    ...
    return Response.redirect(
      new URL(flowValidation.redirectTo, req.nextUrl.origin)
    );
  } catch (error) {
    console.error("[GitHub Callback] OAuth callback error", error);
    return new Response("Failed to complete GitHub authentication", {
      status: 500,
    });
  }
}
...
function validateOAuthState(params: {
  ...
  const { code, state, storedState, storedRedirectTo, storedUserId, flowType } =
    params;
  ...
  const existingAccount = await db
    ...
    .where(
      and(
        ...
        eq(accounts.externalUserId, `${params.githubUser.id}`)
      )
    )
    .limit(1);
...
async function transferUserResources(params: {
  sourceUserId: string;
  targetUserId: string;
}) {
  await db
    .update(tasks)
    .set({ userId: params.targetUserId })
    .where(eq(tasks.userId, params.sourceUserId));
  await db
    .update(connectors)
    .set({ userId: params.targetUserId })
    .where(eq(connectors.userId, params.sourceUserId));
  await db
    .update(accounts)
    .set({ userId: params.targetUserId })
    .where(eq(accounts.userId, params.sourceUserId));
  await db
    .update(keys)
    .set({ userId: params.targetUserId })
    .where(eq(keys.userId, params.sourceUserId));
}

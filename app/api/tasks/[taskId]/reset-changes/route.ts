...
export async function POST(request: NextRequest, { params }: { params: Promise<{ taskId: string }> }) {
  try {
    ...
    const resetTarget = await determineResetTarget(sandbox, context.task.branchName)

    await runGitCommandStrict(sandbox, ['reset', '--hard', resetTarget], 'Failed to reset changes')

    try {
      await runGitCommandStrict(sandbox, ['clean', '-fd'], 'Failed to clean workspace')
    } catch {
    ...
    return handleTaskRouteError(error, 'An error occurred while resetting changes', {
...
async function snapshotLocalChanges(sandbox: ActiveSandbox, commitMessage?: string) {
  const statusOutput = await runGitCommandStrict(sandbox, ['status', '--porcelain'], 'Failed to check status')
  ...
  const message =
    typeof commitMessage === 'string' && commitMessage.length > 0 ? commitMessage : 'Checkpoint before reset'
  await runGitCommandStrict(sandbox, ['commit', '-m', message], 'Failed to commit changes')

  return true
}

async function determineResetTarget(sandbox: ActiveSandbox, branchName: string) {
  ...
  await runGitCommandStrict(sandbox, ['fetch', 'origin', branchName], 'Failed to fetch from remote')
  return 'FETCH_HEAD'
}

async function runGitCommandStrict(sandbox: ActiveSandbox, args: string[], errorMessage: string) {

...
export async function buildTaskRouteContext(options: BuildTaskRouteOptions): Promise<TaskRouteContext> {
  ...
  if (options.requireGitHubClient) {
    ...
    if (!octokit.auth) {
      throw new TaskRouteError('GitHub authentication required. Please connect your GitHub account.', 401)
    }
...
export function handleTaskRouteError(error: unknown, fallbackMessage: string, statusMessages?: Record<number, string>) {
...
function findStatusMessage(error: unknown, messages: Record<number, string>): [number | null, string | null] {
...
export async function getActiveSandbox(taskId: string, sandboxId: string | null) {
  if (!sandboxId) {

...
import { buildTaskRouteContext, getActiveSandbox, handleTaskRouteError } from '../task-route-helpers'
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const { task } = context
    const { commitTitle, commitMessage, mergeMethod = 'squash' } = await request.json()

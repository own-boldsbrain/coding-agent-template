...
export async function POST(request: Request, { params }: { params: Promise<{ taskId: string }> }) {
  try {
    ...
    return handleTaskRouteError(error, 'An error occurred while syncing changes', {
...
async function parseRequestBody(request: Request): Promise<{ commitMessage?: string }> {
  try {
...
async function checkForPendingChanges(sandbox: ActiveSandbox) {
  const result = await runGitCommand(sandbox, ['status', '--porcelain'], 'Failed to check status')
...
async function commitChanges(sandbox: ActiveSandbox, commitMessage?: string) {
  ...
  await runGitCommand(sandbox, ['commit', '-m', message], 'Failed to commit changes')
}

async function pushChanges(sandbox: ActiveSandbox, branchName: string) {
  await runGitCommand(sandbox, ['push', 'origin', branchName], 'Failed to push changes')
}

async function runGitCommand(sandbox: ActiveSandbox, args: string[], errorMessage: string) {
...
async function logGitError(result: Awaited<ReturnType<ActiveSandbox['runCommand']>>) {
  try {

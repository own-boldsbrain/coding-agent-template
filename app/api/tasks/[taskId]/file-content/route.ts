...
async function validateAndParseRequest(
  ...
  const rawMode = searchParams.get("mode") || "remote";
  const mode = rawMode === "local" ? "local" : "remote";
  ...
  if (!octokit.auth) {
    throw new Error(
      "GitHub authentication required. Please connect your GitHub account to view files."
    );
  }
...
function getStatusFromErrorMessage(
  message: string,
...
export async function GET(
  ...
  try {
    ...
    try {
      const validated = await validateAndParseRequest(
        request,
        taskId,
        session.user.id
      );

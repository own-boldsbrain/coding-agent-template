...
async function copyFilesRecursively(
  ...
  try {
    const contents = await fetchDirectoryContents(octokit, sourceOwner, sourceRepoName, sourcePath)
...
async function fetchDirectoryContents(octokit: Octokit, owner: string, repo: string, path: string) {
...
async function copyFileItem(params: {
  ...
  try {
    ...
    const relativePath = getRelativePath(params.item.path, params.basePath, params.item.name)
...
function getRelativePath(path: string, basePath: string, fallbackName: string) {
  ...
  return path.startsWith(`${basePath}/`) ? path.substring(basePath.length + 1) : fallbackName
}
...
async function populateRepoFromTemplate(octokit: Octokit, repoOwner: string, repoName: string, template: RepoTemplate) {
  ...
  const cloneMatch = /github\.com\/([\w-]+)\/([\w-]+?)(?:\.git)?$/.exec(template.cloneUrl)
  if (!cloneMatch) {
...
export async function POST(request: Request) {
  try {
    ...
    if (!token) {
      throw new HttpError('GitHub token not found. Please reconnect your GitHub account.', 401)
    }
    ...
    await attemptTemplatePopulation(octokit, repo.data.owner.login, repo.data.name, payload.template)
...
async function createRepository(octokit: Octokit, payload: Awaited<ReturnType<typeof parseCreateRepoBody>>) {
  ...
  try {
    ...
    if (isGitHubError(error, 404)) {
      throw new HttpError('Organization not found or you do not have permission to create repositories', 403)
    }
